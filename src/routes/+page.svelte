<div class="flex flex-col w-full max-w-screen-xl px-2 md:px-6 mt-4">
	<div class="flex flex-wrap items-center justify-center">
		<div class="md:w-2/5 flex flex-col items-center">
			<h1 class="text-3xl md:text-4xl lg:text-5xl font-bold font-title mb-4 md:mb-6">
				Time to <span class="text-accent">spare</span> <br />
				<span class="md:text-5xl lg:text-6xl">your time</span>
			</h1>
			<p class="mb-4 px-6 text-center md:text-lg">
				Learn how much time you spend on your daily activities, and start making great plans
			</p>
			<a
				href="/activities"
				class="rounded-full px-6 py-3 text-white bg-main-dark hover:bg-main transition-colors duration-300 mb-4 md:mb-6"
			>
				Start now
			</a>
		</div>

		<img src="/Hero.png" alt="Timer Buddy" width="980" height="924" class="w-full md:w-3/5" />
	</div>
</div>
